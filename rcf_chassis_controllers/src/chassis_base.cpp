...
#include "rcf_chassis_controllers/chassis_base.h"
...
namespace rcf_chassis_controllers {
    bool ChassisBase::init(hardware_interface::RobotHW *robot_hw, ros::NodeHandle &root_nh,
        ...
        if (!controller_nh.getParam("publish_rate", publish_rate_) || !controller_nh.getParam("timeout", timeout_)) {//get.param，有数值返回1无返回0
            ROS_ERROR("Some chassis params doesn't given (namespace: %s)", controller_nh.getNamespace().c_str());//如何实现？
            return false;
        }
        controller_nh.getParam("wheel_radius", wheel_radius_);//“键”“值”
        controller_nh.getParam("wheel_track", wheel_track_);
        controller_nh.param("enable_odom_tf", enable_odom_tf_, true);//看后文，有个ifxxxx
        controller_nh.getParam("wheel_base", wheel_base_);

        XmlRpc::XmlRpcValue twist_cov_list;
        controller_nh.getParam("twist_covariance_diagonal", twist_cov_list);//读了什么数据？

        effort_joint_interface_ = robot_hw->get<hardware_interface::EffortJointInterface>();

        // Setup odometry realtime publisher + odom message constant fields--------------- //不太了解odom如何实现-----------------------------
        odom_pub_.reset(new realtime_tools::RealtimePublisher<nav_msgs::Odometry>(root_nh, "odom", 100));//发布了一个真实时间的消息？利用new开创了一块空间，后面用指针去修改数据？
        odom_pub_->msg_.header.frame_id = "odom";
        odom_pub_->msg_.child_frame_id = "base_link";
        odom_pub_->msg_.twist.covariance = {static_cast<double>(twist_cov_list[0]), 0., 0., 0., 0., 0., 0.,//什么数据？
        ...
        if (enable_odom_tf_) {   //enable_odom_tf_前面设的是true
        ...
        ----------------------------------------------------------------------------------------------------------------------------------
                cmd_chassis_sub_ =         //接受rqt发布指令？？？将其放入cmdchassis
                controller_nh.subscribe<rcf_msgs::ChassisCmd>("command", 1, &ChassisBase::cmdChassisCallback, this);//将command消息传到&函数后赋给 cmd_rt_buffer_
        cmd_vel_sub_ = root_nh.subscribe<geometry_msgs::Twist>("cmd_vel", 1, &ChassisBase::cmdVelCallback, this);//将cmd_vel消息传到&函数后赋给 cmd_rt_buffer_

        return true;
    }
    ------------------------------------------------------------------------------------------------------------------
    void ChassisBase::update(const ros::Time &time, const ros::Duration &period) {
        rcf_msgs::ChassisCmd cmd_chassis = cmd_rt_buffer_.readFromRT()->cmd_chassis_;//读取cmd_rt_butter_的数据cmd_chassis
        geometry_msgs::Twist cmd_vel = cmd_rt_buffer_.readFromRT()->cmd_vel_;//读取cmd_rt_butter_的数据cmd_vel

        if ((time - cmd_rt_buffer_.readFromRT()->stamp_).toSec() > timeout_) {//瓦特？？？？？？什么？
            vel_cmd_.x = 0.;
            vel_cmd_.y = 0.;
            vel_cmd_.z = 0.;
        } else {//--------------------------------------------------------------------------读取cmd chassis和vel的数据
            ramp_x_->setAcc(cmd_chassis.accel.linear.x);//ramp是干什么的？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？
        ...
        updateOdom(time, period);//--------------------------------------------------包括了算法？？？？--------------------更新

        ramp_w_->setAcc(cmd_chassis.accel.angular.z);//这些干什么的？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？
    ...
    void ChassisBase::updateOdom(const ros::Time &time, const ros::Duration &period) {
        ...
        if (enable_odom_tf_) {//前面设的是true
            geometry_msgs::Vector3 linear_vel_odom, angular_vel_odom;
            try {
                odom2base_ = tf_buffer_.lookupTransform("odom", "base_link", ros::Time(0));//为什么还要lookup？已经是已odom为坐标了啊
            } catch (tf2::TransformException &ex) {
                tf_broadcaster_.sendTransform(odom2base_);  // TODO: For some reason, the sendTransform in init sometime not work?
                ROS_WARN("%s", ex.what());
                return;
            }
            odom2base_.header.stamp = time;//为什么不是now（）？？
            // integral vel to pos and angle
            tf2::doTransform(vel_base.linear, linear_vel_odom, odom2base_);//？？？？？？？？什么意思？？？？？
            ...
            tf_broadcaster_.sendTransform(odom2base_);//tf----
        }
